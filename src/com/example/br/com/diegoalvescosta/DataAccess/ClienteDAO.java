...
public class ClienteDAO {
	...
	public int retornaIDPeloNome(String nomeCliente){
		int idCliente = 0;

		
		String[] colunas = { "CODIGO" };
		String[] nomeClient = { nomeCliente };
		String where = "NOME = ?";
		BDUtil bdUtil = new BDUtil(context);
		SQLiteDatabase db = bdUtil.getReadableDatabase();
		
		Cursor c = db.query("CLIENTE", // Nome da tabela
				colunas, // Numero de colunas
				where, // Clausula WHERE
				nomeClient, // Parametros WHERE
				null, // Group para agrupar
				null, // Usando Haven
				null // Ordenando pelo nome
		);
